...
> **〜 “もう撮り逃さない！” AIが最高の一瞬をあとから切り抜くスマートカメラアプリ 〜**

## 【アプリの一言説明】
> [!NOTE]
> **QuickCam**は、撮り逃したくない一瞬をあとからAIが見つけて、SNS用にすぐ切り抜けるスマートカメラアプリです。
...
| **② 既存のカメラアプリでよいと思われるのでは？** | 既存カメラとの差別化を、「押した瞬間の前を保存できる」「AIがベストな切り抜き範囲を提案する」の2点に絞って訴求します。単なる録画アプリではなく、決定的瞬間をあとからQuickCam（クイックキャム）するアプリとして打ち出します。 |
...
### 2. QuickCam画面（メインカメラ画面）
* **コンセプト**: 迷わず待機し、残したい瞬間に即タップできる画面。
* **主要UI要素**: 大きなカメラプレビュー。中央下に配置された、触覚フィードバックを持つ大きなシャッター（QuickCam）ボタン。上部に現在のバッファ秒数表示。右上に設定ボタン。下部に「直近60秒を保持中」などのステータス表示。
* **機能要件**: AVFoundationによるカメラプレビュー。直近30秒〜60秒のリングバッファ録画。音声入力の同時取得。保存ボタン押下時のレビュー画面遷移。端末負荷に応じた画質・フレームレート制御。
* **iOS 18 コントロール Widget**: ロック画面やコントロールセンターから即起動し、直接キャプチャ待機へ移行できるウィジェットボタン（QuickCamControl）を提供。
...
### 4. 設定画面
* **コンセプト**: アプリの動作設定およびストレージ・言語設定を行う場所。
* **主要UI要素**: バッファ時間設定（例: 30秒 / 60秒）、画質設定、キャッシュストレージのクリア。アプリ内多言語選択（英語/日本語をリアルタイムに切り替え可能。デフォルトはOSの言語設定を検出）。
...
### 8.1. デザインコンセプト
> [!IMPORTANT]
> **「Matte Navy Neumorphic Premium Gadget Style」**
> マットで上質なネイビーをベースとした、高級ガジェットのような触覚的リアリティと未来感を両立させた専用デザインです。ダークカラーの安心感とネオモーフィズム（Neumorphism）による「触り心地の良さ」を追求し、操作するたびに愛着がわく洗練されたインターフェースを提供します。

### 8.2. カラースキーム（配色）
* **ベースカラー（背景）**: 深く落ち着きのあるマットネイビー（アッシュブルーやダークブルー寄りのシックなトーン）。
* **サブカラー（埋め込み部）**: さらに明度を落としたインセット用のディープネイビー。スライダーの溝や、トグルスイッチの背景、情報パネルに使用。
* **アクセントカラー（主役）**: 視認性とネオモーフィズムの質感を際立たせるコントラスト用ライトグレー / ホワイト、およびAIハイライトや状態表示に使用されるソフトイエロー / シアン。
* **テキストカラー**: 眩しさを抑え、マットネイビーに美しく馴染むライトブルーグレー〜ホワイト。

### 8.3. UIシェイプと質感（マテリアル）
* **隆起と沈み込み**: ボタンやパネルは背景のマットネイビーから「滑らかに盛り上がっている（外側のソフトシャドウ）」、または「削り取られて沈み込んでいる（内側のインセットシャドウ）」ように表現します。
* **境界線の排除**: 明確な枠線（ボーダー）は極力使わず、光（ハイライト）と影（シャドウ）のグラデーションのみで要素の境界を定義します。
* **底面からの発光（グロー）**: 主要なボタンやスライダーは、パーツの隙間や底面からネオンライトがじんわりと漏れ出ているような光彩（グローエフェクト）を付与し、サイバーな雰囲気を強調します。

### 8.4. タイポグラフィ（文字装飾）
* **フォントファミリー**: モダンで幾何学的なサンセリフ体（例：SF Pro, Inter, Outfitなど）。
* **ウェイト（太さ）**: 極力細め（Light / Regular）を多用し、繊細でミニマルな印象を与えます。強調すべきタイムコードや「QuickCam」のロゴ部分のみ、Boldでコントラストをつけます。
* **余白**: 文字の周囲には十分なネガティブスペース（余白）を取り、要素が詰め込まれている印象を避けます。

### 8.5. アニメーションとインタラクション
* **QuickCamボタン**: 待機中はシアンの光がゆっくりと明滅（呼吸）し、タップした瞬間にボタン全体が深く沈み込み、強いシアンの波紋が広がります。
...
```mermaid
graph TD
    A[Step 1: オンボーディング] -->|価値理解| B[Step 2: QuickCam画面で待機]
    B -->|目の前の出来事を楽しむ| C[Step 3: 決定的瞬間の発生]
    C -->|QuickCamボタンをタップ| D[Step 4: レビュー画面でAI提案]
...
* **Step 6: 保存・共有**
  ...
  * **体験**: 撮影から投稿用クリップ作成までが短時間で完了し、「今すぐ見せたい」欲求を満たせる。保存後はすぐにQuickCam画面へ戻り、次の瞬間に備える。
...
### 動画処理パイプライン
* **動画キャプチャ**: AVFoundationでカメラ映像と音声を取得。直近30秒〜60秒の動画をリングバッファとして一時保存し、QuickCamボタンが押された瞬間に対象範囲をレビュー画面へ渡します。
...
* **Never Miss a Snap!**（最もシンプルで強力なグローバルコピー）
* もう撮り逃さない！
* 最高の一瞬は、あとからSnapする。
* 撮る前に迷わない。撮った後にAIが選ぶ。
* 押した瞬間の“少し前”まで残せる。
...
QuickCamは、単なる動画撮影アプリではありません。ユーザーが本当に欲しい「決定的瞬間」だけを、最短で保存・共有するためのスマートカメラアプリです。
